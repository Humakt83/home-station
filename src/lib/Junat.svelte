<script lang="ts">
  ...
  const JUNAT = 'https://rata.digitraffic.fi/api/v1/live-trains?station=JP&arrived_trains=1&arriving_trains=50&departing_trains=50&departing_trains=5';
  const JUNA = 'https://rata.digitraffic.fi/api/v1/trains'

  function isLegitDeparture(tr) {
    return tr.trainStopping && tr.stationShortCode === 'JP' && tr.type === 'DEPARTURE' && new Date().valueOf() <= new Date(tr.scheduledTime).valueOf();
  }
  ...
  async function fetchTrain(trainNumber: number, dateNow: string): Promise<Train> {
    const res = await fetch(`${JUNA}/${dateNow}/${trainNumber}`, { method: 'GET', headers: { 'Content-Type': 'application/json'}});
  ...
  async function fetchDeparturesForStop(): Promise<Array<Departure>> {
    ...
    const data = await res.json();
    const departureData = data.filter(train => !!train.timeTableRows.find(isLegitDeparture))
      .map(train => {
        const sortedRows = train.timeTableRows.sort((a, b) => a.scheduledTime.valueOf() - b.scheduledTime.valueOf())
    ...
    const sorted = out.sort((a, b) => a.scheduledTime.valueOf() - b.scheduledTime.valueOf()).slice(0, 10);
    const dateNow = formatDate(new Date(), "yyyy-MM-dd");
    return await Promise.all(sorted.map(async (dd) => {
      let train: Train | undefined = trainNumberToCommuterLineId.find(t => t.trainNumber === dd.train.trainNumber);
      if (!train) {
		...
		{:else}
			<ul>
				{#each departures as d}
					<li>
						<span class="time"
							>{formatDate(d.scheduledTime, 'hh:mm')}</span
						>
